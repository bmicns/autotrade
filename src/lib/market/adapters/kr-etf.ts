...
import { mapKrEtfPositionSnapshots } from "../positions";
...
async function loadKrEtfConfig(): Promise<EngineConfig> {
  const [{ getToken }, { getActiveKisConfigForAssetClass }] = await Promise.all([
    import("../../kis/api"),
    import("../../kis/runtime-config"),
  ]);
  const active = await getActiveKisConfigForAssetClass("kr_etf");
  if (!active) {
    throw new Error("kr_etf adapter: KIS runtime config missing");
  }
  const token = await getToken(active.config.appKey, active.config.appSecret);
  return buildReadOnlyConfig(active.config, token);
}

export function normalizeKrEtfUniverse(rows: WatchlistRow[]): MarketInstrument[] {
  return rows
...
export function buildKrEtfOrderPreview(intent: OrderIntent, position?: PositionSnapshot | null): OrderPreview {
...
export const krEtfAdapter: MarketAdapter = {
  assetClass: "kr_etf",
  label: "Korean ETF Adapter",
  capabilities: {
  ...
  async listUniverse(filter): Promise<MarketInstrument[]> {
    ...
    if (error) {
      throw new Error(`kr_etf adapter: watchlist query failed: ${error.message}`);
    }

    const normalized = normalizeKrEtfUniverse((data ?? []) as WatchlistRow[]);
  ...
  async listPositions(): Promise<PositionSnapshot[]> {
    ...
    if (error) {
      throw new Error(`kr_etf adapter: positions query failed: ${error.message}`);
    }

    return mapKrEtfPositionSnapshots((data ?? []) as Array<Record<string, unknown>>);
  },
  async getQuote(symbol: string): Promise<MarketQuote> {
    const { getPrice } = await import("../../engine/kis");
    const config = await loadKrEtfConfig();
    const output = await getPrice(config, symbol);
    const price = Number(output.stck_prpr);
    if (!Number.isFinite(price) || price <= 0) {
      throw new Error(output.__error_message || `kr_etf adapter: invalid quote for ${symbol}`);
    }
  ...
  async previewOrder(intent: OrderIntent, position?: PositionSnapshot | null): Promise<OrderPreview> {
    return buildKrEtfOrderPreview(intent, position);
  },
};
