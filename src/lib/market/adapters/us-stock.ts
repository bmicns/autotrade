...
import { buildDefaultUsQuote, buildUsOrderPreview, getDefaultUsStockUniverse } from "./us-shared";
...
async function loadUsStockConfig() {
  const [{ getToken }, { getActiveKisConfigForAssetClass }] = await Promise.all([
    import("../../kis/api"),
    import("../../kis/runtime-config"),
  ]);
  const active = await getActiveKisConfigForAssetClass("us_stock");
  if (!active) {
    throw new Error("us_stock adapter: KIS runtime config missing");
  }
  const token = await getToken(active.config.appKey, active.config.appSecret);
  return buildUsReadOnlyConfig(active.config, token);
}

async function resolveUsVenue(config: Awaited<ReturnType<typeof loadUsStockConfig>>, symbol: string) {
  ...
  return getKnownUsVenue("AAPL")!;
}

export const usStockAdapter: MarketAdapter = {
  assetClass: "us_stock",
  label: "US Stock Adapter",
  capabilities: {
  ...
  async listUniverse(filter): Promise<MarketInstrument[]> {
    const universe = getDefaultUsStockUniverse();
  ...
  async listPositions(): Promise<PositionSnapshot[]> {
    ...
    const config = await loadUsStockConfig();
    ...
    return mapUsBalanceRowsToPositions({ rows, assetClass: "us_stock" });
  },
  async getQuote(symbol: string): Promise<MarketQuote> {
    if (!shouldUseLiveUsKis()) {
      const base = symbol === "AAPL" ? 210 : symbol === "MSFT" ? 430 : 900;
