...
export default function BottomNav({ active, onChange }) {
  return (
    <nav className="glass-nav fixed bottom-0 left-0 right-0 flex justify-around py-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] z-50 border-t border-white/[0.06]">
      {tabs.map((t) => (
        <button
          key={t.id}
          onClick={() => onChange(t.id)}
          className={`flex flex-col items-center min-w-[56px] gap-0.5 transition-colors ${
            active === t.id ? "text-[#0a84ff]" : "text-[#86868b]"
          }`}
        >
          {t.icon}
