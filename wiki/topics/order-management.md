...
## KIS API 에러 알림 (v6.1 계획)

v6.1에서 KIS API route의 오류 무음 처리를 제거하고 Telegram 즉시 알림을 추가할 예정이다.

| 엔드포인트 | 알림 조건 | 함수 |
|-----------|---------|------|
| `POST /api/kis/token` | KIS 401/403 또는 네트워크 에러 | `sendKISApiErrorAlert({ operation: "token", httpStatus, kisCode, kisMessage })` |
| `GET /api/kis/balance` | KIS 호출 실패 (401, 500, 네트워크 오류) | `sendKISApiErrorAlert({ operation: "balance", httpStatus, kisCode, kisMessage })` |

**보안 원칙**: `appKey`/`appSecret`/`token`은 알림 메시지에 절대 포함하지 않는다. `kisCode`, `kisMessage` (200자 이하)만 포함한다.

### 현재 Silent Catch 위치 (P0 조사 대상)
...
| `src/app/api/kis/token` | - | KIS 호출 실패 | sendKISApiErrorAlert 추가 예정 |
| `src/app/api/kis/balance` | - | KIS 호출 실패 | sendKISApiErrorAlert 추가 예정 |
...
- `docs/01-plan/features/nexio.plan.md` (v6.1 — KIS API 에러 핸들링 계획)
- `docs/02-design/features/nexio.design.md` (v6.1 설계서 — silent catch 목록, sendKISApiErrorAlert 스펙)
