...
- **현재 버전**: v6.5.1 (project-plan.md 기준)
...
8. **KIS 연결 Health Check (v6.1 계획)** — 60초 폴링으로 KIS 연결 상태 실시간 감지 및 이상 시 텔레그램 알림
9. **운영 에러 알림 (v6.1 계획)** — KIS API 오류, 엔진 런타임 에러 발생 시 텔레그램 즉시 알림

## v6.5.1 → v7.0 로드맵 요약 (docs/project-plan.md)

| 단계 | 우선순위 | 항목 |
|------|---------|------|
| 1단계 (즉시) | ★★★ | recordTradeMemory 호출 누락 수정 (STEP 2/3/4) |
| 1단계 (즉시) | ★★★ | 엔진 오류 Telegram 즉시 알림 (`sendEngineErrorAlert`) |
| 1단계 (즉시) | ★★ | KIS 토큰 재발급 재시도 자가 복구 (2회) |
| 1단계 (즉시) | ★★ | 엔진 헬스체크 API (lastRunAt, status) |
| 1단계 (즉시) | ★★ | steps.ts utils 분리 (`batchFetch`, `getOpeningBonus` → utils.ts) |
| 2단계 | ★★★ | 학습 데이터 적재 검증 UI (tradeMemoryCount) |
| 2단계 | ★★★ | 백테스트 vs 실전 비교 |
| 2단계 | ★★ | 전략별 strategyKey 전달 |
| 2단계 | ★★ | signal-tab.tsx 분리 (~80줄 목표) |
| 2단계 | ★★ | strategy-tab.tsx 분리 (~150줄 목표) |
...
- `docs/02-design/features/nexio.design.md` (v6.1 설계서)
