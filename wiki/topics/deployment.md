...
| 변수명 | 용도 | 등록 위치 | v7.1 필수 여부 |
|--------|------|----------|--------------|
| `CRON_SECRET` | `/api/engine`, `/api/daily-report` GET 인증 | Vercel 환경변수 + GitHub Actions Secret | **필수** (auth 그룹) |
| `ADMIN_SECRET` | 관리자 인증 | Vercel 환경변수 | **필수** (auth 그룹) |
| `ADMIN_PASSWORD` | 관리자 비밀번호 | Vercel 환경변수 | **필수** (auth 그룹) |
| `NEXT_PUBLIC_SUPABASE_URL` | Supabase 연결 | Vercel 환경변수 | **필수** (supabase 그룹) |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Supabase 연결 | Vercel 환경변수 | **필수** (supabase 그룹) |
| `TELEGRAM_BOT_TOKEN` | 텔레그램 봇 인증 | Vercel 환경변수 | **필수** (telegram 그룹) |
| `TELEGRAM_CHAT_ID` | 텔레그램 수신 채팅 ID (`7224554519`) | Vercel 환경변수 | **필수** (telegram 그룹) |
| `KIS_APP_KEY` | KIS 초기 복구 / 부트스트랩용 | Vercel 환경변수 | warn (DB 대체 가능) |
| `KIS_APP_SECRET` | KIS 초기 복구 / 부트스트랩용 | Vercel 환경변수 | warn (DB 대체 가능) |
| `KIS_ACCOUNT_NO` | KIS 초기 복구 / 부트스트랩용 | Vercel 환경변수 | warn (DB 대체 가능) |
| `DART_API_KEY` | DART 공시 필터 API | Vercel 환경변수 | 선택 |

**v7.1 환경변수 검증 그룹** (`src/lib/config-validator.ts`):
- `REQUIRED_ENV_GROUPS`: supabase, auth, telegram — 누락 시 엔진 500 반환 + Telegram 알림
- `WARN_ENV_GROUPS`: kis — 누락 시 warn 로그만 (DB `kis_config`로 대체 가능)
...
**v7.1 신뢰 시스템 (2026-05-02)**

운영 안정성과 보안 취약점 해결을 위한 4개 개선:
1. 환경변수 누락 자동 감지 — 엔진 진입 즉시 검증, 누락 시 Telegram 알림 후 조기 종료
2. balance/price API POST 전환 — `appSecret` URL 노출 제거
3. 엔진 자가복구 (중복 실행 방지 + 고립 정리 + 지수 백오프 재시도)
4. 핵심 경로 E2E 테스트 (`tests/core.spec.ts`, Playwright) — 회귀 자동 감지
...
```
playwright.config.ts        — baseURL, testDir, storageState 설정
tests/global.setup.ts       — 로그인 후 .auth/user.json 저장 (storageState 생성)
tests/core.spec.ts          — Playwright 핵심 경로 테스트 (TC-01/02/03)
tests/.auth/user.json       — 인증 상태 캐시 (gitignore 권장)
```

**설계 초과 구현 사항**: 설계서에 명시되지 않은 `global.setup.ts`가 추가됨. 로그인 플로우를 분리하여 `core.spec.ts`의 모든 테스트가 사전 인증 상태를 재사용한다.

### 테스트 케이스

| TC | 이름 | 전제 조건 | 검증 항목 | 통과 기준 |
|----|------|---------|---------|---------|
| TC-01 | 로그인 | 앱 실행 중 | 관리자 ID/PW 입력 → 제출 | 대시보드 URL + 주요 UI 요소 표시 |
| TC-02 | 대시보드 렌더링 | TC-01 완료 | 잔고, 포지션 영역 표시 | 에러 바운더리 미발동 + 영역 DOM 존재 |
| TC-03 | 설정 표시 | TC-01 완료 | 설정 탭 진입 | KIS 설정 폼(appKey 입력) DOM 존재 |

**환경 분리 원칙**: E2E 테스트는 staging/local 환경에서만 실행. `PLAYWRIGHT_BASE_URL` 환경변수로 대상 URL 주입. 프로덕션 KIS 계정 사용 금지.
...
- `docs/01-plan/features/nexio.plan.md` (v7.1 — 환경변수 자동 감지, POST 전환, 자가복구, E2E 테스트 계획)
- `docs/02-design/features/nexio.design.md` (v7.1 설계서 — config-validator.ts, retry.ts, E2E 테스트 케이스 상세)
- `docs/plan-v6.md` (v6.0 미구현 기능 순차 구현 로드맵 — v5.9→v6.0 전환 계획)
