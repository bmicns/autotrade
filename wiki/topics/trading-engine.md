...
| `src/lib/engine/steps.ts` | STEP 0~3 실행 로직, `batchFetch`, `getOpeningBonus` | 466줄 |
| `src/lib/engine/notify.ts` | 텔레그램 알림 (`sendTradeAlert`, `sendDailyReport`) | ~90줄 |
| `src/lib/engine/types.ts` | `EngineConfig`, `EngineAction`, `StepContext`, `MarketTrend` 등 | ~70줄 |
...
- 익절(`take_profit`) 조건 충족 시 `takeProfitRatio`% 분할 매도(기본 50%). 손절/트레일링 시 전량 매도.
...
- 매수 성공 시 `openPosition()` + `sendTradeAlert({ type: "buy" })` + `status='expired'` 처리.
...
- `adjustedScore ≥ 70` → **strong**: 실시간 가격 조회 → 종목 필터 → DART 공시 필터 → `limitBuyOrder()` → `openPosition()` + `recordTradeMemory()` + `sendTradeAlert({ type: "buy" })`
...
STEP 2와 동일한 `batchFetch` + strong/weak 판정 로직을 적용한다. strong 매수 시 `positionSize × 0.5 / price`로 1차 매수만 실행. snapshot 보너스는 적용하지 않는다. 매수 성공 시 `sendTradeAlert({ type: "surge_buy" })`.
...
- `src/lib/engine/steps.ts` (STEP 0~3 실행 로직)
