...
NEXIO의 신호 시스템은 7종의 기술 지표를 가중치 점수제로 합산하고, 시장·투자자·장 초반 스냅샷 보정 점수를 더해 종목별 매수·매도 강도를 수치화한다. 점수에 따라 즉시 지정가 주문(strong), 사용자 승인 대기(weak), 무시(none) 세 가지 경로로 분기한다. 기본 가중치(`analyzeSignal`)와 학습 가중치(`analyzeSignalWithWeights`) 두 경로를 병렬 실행해 A/B 비교 데이터를 누적한다.
...
- `src/lib/kis/indicators.ts` — `analyzeSignal`, `analyzeSignalWithWeights`, `calcATR`, `calcDynamicRisk`, `calcPositionSize`, `checkRisk`
