...
## SignalTab 구조 변경 (v6.1 계획)

설계서(`docs/02-design/features/nexio.design.md`) 기준 SignalTab의 역할이 "신호 승인/거절 전용"으로 순수화된다.
...
변경 후: SignalTab
  └── 신호 대기 (PendingSignalList, ManualBuyForm)  ← 전용화
       (관심종목 탭 제거 → StrategyTab 하단 WatchlistSection으로 이동)
```

### 신규 훅 계획 (v6.1)

컴포넌트 내 직접 fetch 로직이 전용 훅으로 분리될 예정이다.

| 훅 | 파일 | 담당 API |
|----|------|---------|
| `useStockSearch` | `src/hooks/useStockSearch.ts` | `/api/stock-search` (debounce 300ms) |
| `useThresholdsOptimize` | `src/hooks/useThresholdsOptimize.ts` | `/api/optimize-thresholds` |
| `usePositions` | `src/hooks/usePositions.ts` | `/api/positions` |
| `usePortfolioSnapshot` | `src/hooks/usePortfolioSnapshot.ts` | `/api/portfolio-snapshot` |
| `useNews` | `src/hooks/useNews.ts` | `/api/news` |
...
- `docs/01-plan/features/nexio.plan.md` (v6.1 Signal/Strategy 탭 구조 개선 계획)
- `docs/02-design/features/nexio.design.md` (v6.1 설계서 — SignalTab 순수화, useStockSearch)
